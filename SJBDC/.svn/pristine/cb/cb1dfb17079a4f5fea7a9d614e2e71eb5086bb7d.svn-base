	 ...
	 var filterPanel = new FilterPanel({
         ...
         callback: function () {
        	var starttime=$('#startDate').val();
     		...
     		var url = '/SJBDC/commonController/queryList.do?queryId=JgXnSjjhQueryMapper.querySJHJ&queryMap[cityCode]='+cityCode+'&queryMap[starttime]='+starttime+'&queryMap[endtime]='+endtime;
	...
	$('#resultList').bootstrapTable({
		...
		onLoadSuccess: function(result) {
			...
			for ( var i=0;i<result.length;i++) {
				ZL += result[i].HJZS;
				HJCGSL += result[i].HJCGSL;
				HJWFKSL += result[i].HJWFKSL;
				HJYCSL += result[i].HJYCSL;
			...
			var data1=[];
			var data2=[];
			data1.push({name:'正常',value:HJCGSL},{name:'异常',value:HJYCSL},{name:'未反馈',value:HJWFKSL});
		...
		columns : [ {
			...
			align : 'center',
			formatter : function(value, row, index) {
				if(row.HJCGSL==''||row.HJCGSL==null)
				{
					return 0;
				}
				else{
					return '<a  onClick="query('+row.QXDM+')">'+row.HJZS+'</a>';
				}
				}
		}, {
			field : 'HJCGSL',
		...
		},
		{
			...
			formatter: function (value, row, index) {
            	...
            	return parseInt((row.HJCGSL/row.HJZS)*100)+"%";
            	}
			}
		]
...
function query(cityCode){
	location.href=("sjhjPage7.jsp?xzqdm="+cityCode);
}