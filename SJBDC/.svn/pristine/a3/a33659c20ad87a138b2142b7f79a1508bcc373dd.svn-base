...
jQuery.validator.addMethod("chinaZip", function(value, element) {
	return this.optional(element) || /^\d{6}$/.test(value);
}, "无效的邮政编码");
jQuery.validator.addMethod("futureNow", function(value, element) {
	var temp =new Date();
	var currentDate = new Date(temp.getFullYear()+"/"+(temp.getMonth()+1)+"/"+temp.getDate());
	var selectedDate = new Date(value.replace(/-/g, "/"));
	return this.optional(element) || currentDate <= selectedDate;
}, "选择日期需大于等于在当前日期");
