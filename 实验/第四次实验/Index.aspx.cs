...
public partial class _Index : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)//这个函数最重要，所有页面的动态样子都在这里决定
    {
        ...
        if (!IsPostBack)//如果是全页的刷新，新加载或者F5，ReDirect
        {
            ...
            {
                if (TotalPage == 1)
                    CmdStr = "Select * from mesboard order by Id";
                else if (TotalPage > 1)
                    CmdStr = "Select top " + PageSize + " * from mesboard order by Id";
            }
            else if (CurPage > 1)
            {
                string CmdStr1 = "Select MAX(Id) from (Select top " + (CurPage - 1) * PageSize + " Id from mesboard order by Id )";
                OleDbCommand Cmd1 = new OleDbCommand(CmdStr1, OleDbCon);
                int MZ = Convert.ToInt16(Cmd1.ExecuteScalar());

                if (CurPage == TotalPage)
                    CmdStr = "Select * from mesboard where Id>" + MZ + " order by Id ";
                else
                    CmdStr = "Select top " + PageSize + " *  from mesboard where Id>" + MZ + " order by Id ";
            }
